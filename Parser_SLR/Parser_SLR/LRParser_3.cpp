...
template<typename T>
unordered_set<string>*  LRParser_3<T>::closure(unordered_set<string>& Items)
{
	//先计算后判断是否已存在;
...
template<typename T>
unordered_set<string>* LRParser_3<T> ::_goto(int In, string input)
{
	// 从映射寻找函数goto是否已经存在
...
template<typename T>
int LRParser_3<T>::checkExist_In_SISF(unordered_set<string>& NewItemSet)
{
...
template<typename T>
string LRParser_3<T>::getOneItem(string raw, vector<string>& rawProd, int point)
{
	// 创建新的项目
...
template<typename T>
void LRParser_3<T>::LR0_build(unordered_set<string>& TokenList, int In)
{
...
template<typename T>
void LRParser_3<T>::parser_build()
{
...
LRParser_3<T>::LRParser_3()
{
	...
	init_values = new unordered_set<string>();

	semanticRules = new unordered_map<int, Semantic<T>*>();
}
...
LRParser_3<T>::~LRParser_3()
{
	...
	for (unordered_map<int, Semantic<T>*>::iterator it = semanticRules->begin(); it != semanticRules->end(); ++it)
	{
		delete it->second;
	}
	delete semanticRules;
}

template<typename T>
bool LRParser_3<T>::isEnabled()
{
	return enabled;
}

template<typename T>
bool LRParser_3<T>::init(
...
template<typename T>
void LRParser_3<T>::add_ignorableErr(string symbol, string comment)
{
	// 查询整个分析表,找到不接收符号的位置
	for (unordered_map<int, unordered_map<string, Operation*>*>::iterator it = action_stat_input__act->begin(); it != action_stat_input__act->end(); ++it)
	{
...
template<typename T>
void LRParser_3<T>::add_initVals(string symbol)
{
	init_values->insert(symbol);
}

template<typename T>
void LRParser_3<T> ::add_semanticRules(int prod_index, vector<int>& prodItem_index, T(*opration)(T, T))
{
	Semantic<T>* nSemantic = new Semantic<T>();
	semanticRules->insert(pair<int, Semantic<T>*>(prod_index, nSemantic));
	nSemantic->opration = opration;
	nSemantic->pos1 = prodItem_index[0];
	nSemantic->pos2 = prodItem_index[1];
}

template<typename T>
void LRParser_3<T>::parse(const char * fileName, bool analyze, ostream& output)
{
	if (!enabled)
	{
	...
	for (int itt = 0; itt < size; ++itt)
	{
		...
		// 此处判断类型
		string curr = (*tokens)[itt]->token;
		string cur_arg = (*tokens)[itt]->arg;
		...
		case 1: {
			...
			if (analyze)
			{
				if (init_values->count(curr) == 0)
				{	// 将运算符移入(用0代替)
					valStack.push(0);
				}
				else
				{	// 将初值移入
					T v = string2V<T>(cur_arg);
					valStack.push(v);
				}
			}
		...
		case 2: {
			...
			bool needsAnalyze = analyze && (semanticRules->count(o->operate) > 0);		// 如果产生式有语义分析规则,就需要一起弹栈

			string left = rule.substr(0, rule.find("->"));
			//弹栈
			vector<T> vals;			// 临时的一个动态数组,保存并计算val
			for (int p = prod.size() - 1; p >= 0; --p)
			{
				// 是否需要确认？
				statStack.pop();
				tokenStack.pop();
				if (needsAnalyze)					// 有的规约需要进行值的计算
				{
					vals.push_back(valStack.top());
					valStack.pop();
				}
			}
			...
			{
				{
					Semantic<T>& s = *semanticRules->at(o->operate);
					output << "语义分析:本次规约需要执行执行计算操作" << endl << string() << endl;
					valStack.push(
						s.opration(
							vals[vals.size() - s.pos1 - 1],
							vals[vals.size() - s.pos2 - 1]
						)
					);
				}
			}
